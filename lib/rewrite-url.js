...
var defaultConfig = {
	 ...
	 * которого будем высчитывать абсолютные ссылки из документа.
	 */
	root: process.cwd(),
	...
	validUrl: function(url, node) {
		// @deprecated: использовать атрибут data-href="preserve"
		if (node.name === 'a' && node.attribs['data-link'] === 'absolute') {
			return false;
		}
...
module.exports = function(config) {
	...
	return function(dom, res) {
		findNodesToRewrite(dom, config).forEach(function(item) {
			var absUrl = absoluteUrl(item.node.attribs[item.attribute], res.origin, config.cwd);
			...
			if (config.transform) {
				targetUrl = config.transform(targetUrl, {
					...
					actual: path.join(config.cwd, absUrl),
					context: item,
					absoluteUrl: function(url) {
						return absoluteUrl(url, res.origin, this.cwd);
					},
					rebuildUrl: function(url) {
						return rebuildUrl(url, this.prefix);
					}
				});
			}
